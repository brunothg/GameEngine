...
/**
 * 
 * This class represents a nine pitch image. Such an image contains a 1px border. Black
 * pixel({@link Color#BLACK} ) at the left or top border indicate the stretch regions. Relative size
 * of multiple regions will be kept.
 * 
 * @author Marvin Bruns
 *
 */
public class NinePatchImage implements Image
{
	...
	/**
	 * Crates a new stretchable image from a nine patch image. The data array is shared (consider
	 * creating a copy of the image).
	 * 
	 * @see BufferedImage#getSubimage(int, int, int, int)
	 * @param src The shared source image
	 */
	...
	private void compile(BufferedImage src)
	{
		...
		for (Patch h : hPatches)
		{
			for (Patch v : vPatches)
			{
				...
				region.img = src.getSubimage(h.start, v.start, h.length(), v.length());
	...
	private int getInset(BufferedImage src, boolean horizontal, boolean positive)
	{
		...
		int count = 0;
		for (int pos = start; ((positive) ? pos < size - 1 : pos > 0); pos = ((positive) ? pos + 1 : pos - 1))
		{

			Color c = new Color(
				src.getRGB((horizontal) ? pos : src.getWidth() - 1, (horizontal) ? src.getHeight() - 1 : pos), true);
	...
	private List<Patch> getPatches(BufferedImage src, boolean horizontal)
	{
		...
		for (int pos = 1; pos < size - 1; pos++)
		{
			...
			Color c = new Color(src.getRGB((horizontal) ? pos : 0, (horizontal) ? 0 : pos), true);
			if (!c.equals(Color.BLACK) || pos == size - 2)
			{
				if (start != -1)
				{
					...
					Patch p;
					{
						Patch last = (patches.size() > 1) ? patches.get(patches.size() - 1) : null;
		...
		{
			Patch last = (patches.size() > 0) ? patches.get(patches.size() - 1) : null;
	...
	public void draw(Graphics2D g, int width, int height)
	{
		...
		int usedNaturalHeight = (naturalHeight <= height) ? naturalHeight : height;
		...
		if (usedRelativeWidth < 1 || usedRelativeHeight < 1)
		{
			...
			if (usedRelativeWidth < usedRelativeHeight)
			{
				...
				usedNaturalHeight = (int) Math.min(Math.round(naturalHeight * usedRelativeHeight), height);
			}
			else
			{

				usedRelativeWidth = usedRelativeHeight;
				usedNaturalWidth = (int) Math.min(Math.round(naturalWidth * usedRelativeWidth), width);
			}
		}
		...
		for (int y = 0; y < vSizes.length; y++)
		{
			...
			for (int x = 0; x < hSizes.length; x++)
			{
				...
				g.drawImage(img, (int) Math.round(posX), (int) Math.round(posY), (int) Math.round(posX + (_width)),
					(int) Math.round(posY + (_height)), 0, 0, img.getWidth(), img.getHeight(), null);
	...
	/**
	 * Get the natural minimum size of this image. Natural means the pixel of the image, that aren't
	 * marked as stretch area.
	 * 
	...
	/**
	 * Get the {@link Insets} specified by the underlying nine patch source image.
	 * 
	...
	/**
	 * Get the source size of this image. The source size is the size the underlying image provides
	 * without the patch indicator border. This results in the a size of:<br>
	 * <br>
	 * <code>src.getWidth() - 2, src.getHeight() - 2</code><br>
	 * <br>
	 * As the insets are returned as absolute values calculated with the source size you can
	 * calculate the relative insets with the source size.
	 * 
	...
	/**
	 * Set the interpolation quality values from 0 to 2 are allowed. Values out of the range will be
	 * increased/decreased to fit the range.
	 * 
